...
Of the dataset, there are two values that have especially strong positive correlation. Voter turnout strongly positively correlates with a high percentage of white county members, and income has a positive correlation with a percentage of voters who have a bachelor's degree or higher. To me, both of these are unsurprising, given the country is a majority white and that people who are more educated tend to be more politically involved. On the opposite, the percentage unemployed has a negative correlation with voter turnout and the percentage of white county members has a negative correlation with the percentage of unemployed county members. Given deep-rooted segregation and institutional racism in our country, both of these unfortunately checks out. The rest seem to have weak correlations with each other. However, the most interesting is the median age with the percent who have a bachelor's degree or higher, which has little to no correlation. This makes sense because most people get their degree at 22. 
...
I would say that this model predicts turnout fairly well. For one, its p-value is so small that it's in scientific notation, indicating statistical significance. Its multiple R-squared tells us that 85% of the data's variability is explained by the model. Its adjusted R-squared value tells us that 80% of the data is accounted for in this model. Both figures are supermajorities. Finally, the min and max residuals are both less than 1 , meaning it's relatively accurate to voter turnout. 
...
To be honest, I wasn't able to find a combination that made a significant improvement on top of one another. My initial instinct was to get rid of either income or median age because those don't seemt o have any significant correlatons, but in some cases, the models became less accurate with their exclusion. This may be a good thing, because it shows all the data is necessary to the correlation matrix if excluding any of them doesn't garner significant improvements to the adjusted R-squared.

```{r}
# Model 2
model2 <- lm(turnout ~ turnout + population + pct_bachelors_higher + pct_unemployed + pct_white, data = md_turnout)
summary(model2)

# Model 3
model3 <- lm(turnout ~ turnout + population + income + pct_bachelors_higher + pct_unemployed + pct_white, data = md_turnout)
summary(model3)

# Model 4
model4 <- lm(turnout ~ turnout + population + income + pct_white, data = md_turnout)
summary(model4)
...
For one, I think this model shows a multifaceted approach to voter turnout. A large problem with using any one of these stats as a linear regression model only is that they all interact with each other, so having a correlation matrix can show the bigger picture. As for its practical application, this can be used in two ways -- to predict and compare voter turnout for an upcoming election. With these stats, you can get a general baseline for what turnout for each county should look like. Then, when you get the actual stats for turnout, you can then compare them to see what trends have changed since the last cycle. 
