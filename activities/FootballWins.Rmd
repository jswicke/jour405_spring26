...
```{r}
model <- lm(Wins ~ Giveaways + Takeaways + `Yards Allowed` + `Points Scored`, data = teams)
summary(model)
```
...
The model is useful, but it should be acknowledged that there are significant flaws in its data collection. With a min-max of 4 games and a P-squared value of 58%, it's clear that there could be sizable gaps in the data that the model cannot account for. With this in mind, it's still useful as a baseline, but it has its limitations. 
...
```{r}
selected_vars <- teams |>  select(`Yards Allowed`, Takeaways, Giveaways, `Points Scored`, Wins)
cor(selected_vars)
```
...
After readjusting the model I found that "yards gained" actually dilutes the data quite a bit, and its inclusion significantly lowers the adjusted r-squared. I wish I knew enough about football to say why, but I'd assume it's because scoring yards doesn't necessarily give you a win since the other team can just score more points than you if your defense sucks. Conversely, yards allowed makes a major difference, given its significant spike in adjusted r-squared and its large coefficient in the correlation matrix. Points scored obviously is a major factor too. Takeaways and giveaways only slightly adjust the model and only slightly affect wins. 
...
You can use this model to predict if a team will win or lose based on the stats used in this example. 
